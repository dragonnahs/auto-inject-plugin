...
let astLinks = [], astScripts = []
function judgeIsLoadedCss(ast) {
  ast.map(item => {
    if (item.name === 'link' && item.type === 'tag' && item.attrs.rel === 'stylesheet') {
      astLinks.push(item)
    }
    if (item.name === 'script' && item.type === 'tag') {
      astScripts.push(item)
    }
    if (item.children && item.children.length > 0) {
      judgeIsLoadedCss(item.children)
    }
  })
  return { astLinks, astScripts }
}
...
<<<<<<< HEAD
    ...
    let linkName = lastUnit.substring(0, lastUnit.indexOf('.'))
    let linkExt = lastUnit.substring(lastUnit.lastIndexOf('.') + 1)
    if (!astLinks.some(item => item.attrs.hasOwnProperty(linkName)) && linkExt === 'css') {
      html = html.replace('</head>', '<link tag=auto href="' + link + '" rel=stylesheet>' + '</head>')
    }
    if (!astScripts.some(item => item.attrs.hasOwnProperty(linkName)) && linkExt === 'js') {
      html = html.replace('</body>', '<script tag=auto src=' + link + '></script>' + '</body>')
=======
    // css inject
...
>>>>>>> ba47f83cefd7f73cf448ddd5277476706fbecd34
    }
  })
